...
if __name__ == '__main__':
    ...
    for videoID in range(len(video_files)):
        ...
        if os.path.exists(mat_save) and (len(os.listdir(mat_save)) > 0):   #
            print('voxel and files exist, pass')
            continue
        ...
        for FileID in tqdm(range(len(fileLIST))):
            ...
            #     print('without aedat4 file, pass it.')
            #     continue
            else:
                continue
            ...
            for frame_no in range(0, int(frame_num / dvs_img_interval) - 1):
                ...
                if start_idx == end_idx:
                    ...
                    scipy.io.savemat(mat_save + 'frame{:0>4d}.mat'.format(frame_no), mdict={'coor': np.zeros([save_voxel, 3]),
                                     'features': np.zeros([save_voxel, 16])})  # coor: numpy.nan;   features: numpy.nan
